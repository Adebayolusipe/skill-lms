...
class Command(BaseCommand):
    ...
    def compress_image(self, input_path: Path, output_path_prefix: Path, quality: int, no_webp: bool):
        """
        I wrote this function to compresses an image, saving an optimized version and optionally a WebP version.
        It returns paths to generated files.
        """
        try:
            img = Image.open(input_path).convert("RGB") # Converted to RGB to handle transparency consistently

            output_files = []
            original_stem = output_path_prefix.stem
            original_suffix = output_path_prefix.suffix

            # 1. Optimized original format (JPEG/PNG)
